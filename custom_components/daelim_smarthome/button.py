...
async def async_setup_entry(
    ...
    """Set up Daelim buttons."""
    data = hass.data[DOMAIN][entry.entry_id]
    client = data["client"]
    entry_id = entry.entry_id
    complex_name = entry.data["complex"]

    elevator_entity = DaelimElevatorButton(
        client,
        entry_id,
        ELEVATOR_DEVICE[0],
        ELEVATOR_DEVICE[1],
    ...
    all_off_entity = DaelimAllOffButton(
        client,
        entry_id,
        ALL_OFF_DEVICE[0],
        ALL_OFF_DEVICE[1],
    ...
    async_add_entities([elevator_entity, all_off_entity])
...
class DaelimAllOffButton(ButtonEntity):
    ...
    def __init__(
        ...
        """Initialize all-off button."""
        self._client = client
        self._entry_id = entry_id
        self._device_id = device_id
        self._attr_name = name
        self._attr_unique_id = f"{entry_id}_{device_id}_all_off"
        self._complex_name = complex_name
